...
static inline cpFloat Sharpness(cpVect a, cpVect b, cpVect c)
{
...
cpPolyline *cpPolylineSimplifyVertexes(cpPolyline *line, cpFloat tol)
{
	...
	if (cpPolylineIsClosed (line) && Sharpness (reduced->verts[reduced->count - 2], reduced->verts[0], reduced->verts[1]) < minSharp)
	{
...
static cpPolyline *DouglasPeucker(
		cpVect *verts, cpPolyline *reduced, int length, int start, int end, cpFloat min, cpFloat tol
)
{
	...
	if (cpvnear (a, b, min) && cpPolylineIsShort (verts, length, start, end, min))
		return reduced;
...
static int cpPolylineSetFindEnds(cpPolylineSet *set, cpVect v)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (cpveql (line->verts[line->count - 1], v))
			return i;
	}
...
static struct Notch DeepestNotch(int count, cpVect *verts, int hullCount, cpVect *hullVerts, int first, cpFloat tol)
{
	struct Notch notch = {};
	...
	if (hullCount != count)
	{
		...
		if (notch.d > tol)
		{
			...
			if (steiner_it >= 0.0)
			{
				...
				for (int i          = 0; i < sub2_count; i++)
				{
					scratch[i] = verts[(steiner_i + 1 + i) % count];
				}
