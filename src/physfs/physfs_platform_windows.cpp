...
static char *unicodeToUtf8Heap(const WCHAR *w_str)
{
    ...
    if (w_str != NULL)
    {
        ...
        retval = static_cast<char *>(allocator.Malloc(len));
...
static DWORD pollDiscDrives(void)
{
    ...
    if (lib)
        stem = (fnSTEM) GetProcAddress(reinterpret_cast<HMODULE>(lib), "SetThreadErrorMode");
...
char *__PHYSFS_platformCalcPrefDir(const char *org, const char *app)
{
    ...
    retval = reinterpret_cast<char *>(allocator.Malloc(len));
    if (!retval)
    {
        allocator.Free(utf8);
        BAIL(PHYSFS_ERR_OUT_OF_MEMORY, NULL);
...
char *__PHYSFS_platformCalcUserDir(void)
{
    ...
    pGetDir=(fnGetUserProfDirW) GetProcAddress(reinterpret_cast<HMODULE>(lib),"GetUserProfileDirectoryW");
